...
io.on('connection', function (socket) {
  ...
  socket.on('reject-call', username => onRejectCall(socket, username));
  socket.on('accept-call', username => onAcceptCall(socket, username));
  console.log(`${Date(Date.now()).toLocaleString()}: new user connected`);
  socket.on('disconnect', () => onDisconnect(socket));
});

onAcceptCall = (socket, username) => {
  if (io.users[username]) io.to(io.users[username].socketId).emit('accepted-call', io.users[socket.username]);
};

onRejectCall = (socket, username) => {
  if (io.users[username]) {
    io.to(io.users[username].socketId).emit('rejected-call', io.users[socket.username]);
  }
};

onCall = (socket, username) => {
  if (io.users[username]) {
    io.to(io.users[username].socketId).emit('call', io.users[socket.username]);
  } else {
...
onDisconnect = socket => {
  ...
  console.log(`${Date(Date.now()).toLocaleString()} ID:${socket.username} user disconnected`);
  onUsersChange();
};
